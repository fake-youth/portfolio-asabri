    ...
    <div class="table-wrapper">
        <div class="d-flex justify-content-between align-items-center mb-4">
            <h5 class="mb-0" style="color: var(--asabri-navy);">
                <i class="fas fa-calendar-week"></i> Daftar Laporan Bulanan
            </h5>

            <div class="d-flex gap-2">
                <form action="{{ route('laporan.bulanan.index') }}" method="GET" class="d-flex gap-2">
                    ...
                    @if(request('keyword') || request('date'))
                        <a href="{{ route('laporan.bulanan.index') }}" class="btn btn-sm btn-secondary" title="Reset">
                    ...
                    <tbody>
                        @foreach($laporans as $index => $laporan)
                            <tr>
                                ...
                                <td>{{ $laporan->judul }}</td>
                                <td>{{ $laporan->periode_bulan ?? '-' }}</td>
                                <td>{{ $laporan->tanggal_laporan->format('d/m/Y') }}</td>
                                <td>{{ $laporan->uploader->name }}</td>
                                <td>
                                    @auth
                                        <a href="{{ route('laporan.bulanan.download', $laporan->id) }}"
                                            class="btn btn-sm btn-primary" title="Download">
                                            <i class="fas fa-download"></i>
                                        </a>
                                        <a href="{{ route('laporan.bulanan.preview', $laporan->id) }}"
                                    ...
                                    @if(canManage())
                                        <button class="btn btn-sm btn-warning"
                                            onclick="editLaporan({{ $laporan->id }}, '{{ $laporan->judul }}', '{{ $laporan->tanggal_laporan->format('Y-m-d') }}', '{{ $laporan->periode_bulan }}')">
    ...
    @if(canManage())
            ...
            <div class="modal-dialog">
                <div class="modal-content">
                    <div class="modal-header" style="background: var(--asabri-navy); color: white;">
                        <h5 class="modal-title">Upload Laporan Bulanan</h5>
                        <button type="button" class="btn-close btn-close-white" data-bs-dismiss="modal"></button>
                    </div>
                    <form action="{{ route('laporan.bulanan.store') }}" method="POST" enctype="multipart/form-data">
                        @csrf
        ...
        <div class="modal fade" id="editModal" tabindex="-1">
            <div class="modal-dialog">
                <div class="modal-content">
                    <div class="modal-header" style="background: var(--asabri-navy); color: white;">
                        <h5 class="modal-title">Edit Laporan Bulanan</h5>
                        <button type="button" class="btn-close btn-close-white" data-bs-dismiss="modal"></button>
                    </div>
                    <form id="editForm" method="POST" enctype="multipart/form-data" action="">
                        ...
                        <div class="modal-body">
                            ...
                            <div class="mb-3">
                                <label class="form-label">Periode Bulan</label>
                                <input type="text" name="periode_bulan" id="edit_periode" class="form-control">
                            </div>
        ...
        <script>
            function editLaporan(id, judul, tanggal, periode) {
                document.getElementById('editForm').action = "{{ url('laporan-bulanan') }}/" + id;
