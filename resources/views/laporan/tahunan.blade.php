<x-app-layout>
    ...
    <div class="table-wrapper">
        <div class="d-flex justify-content-between align-items-center mb-4">
            <h5 class="mb-0" style="color: var(--asabri-navy);">
                <i class="fas fa-calendar-week"></i> Daftar Laporan Tahunan
            </h5>

            <div class="d-flex gap-2">
                <form action="{{ route('laporan.tahunan.index') }}" method="GET" class="d-flex gap-2">
                    ...
                    @if(request('keyword') || request('date'))
                        <a href="{{ route('laporan.tahunan.index') }}" class="btn btn-sm btn-secondary" title="Reset">
                    ...
                    <tbody>
                        @foreach($laporans as $index => $laporan)
                            <tr>
                                ...
                                <td>
                                    @auth
                                        <a href="{{ route('laporan.tahunan.download', $laporan->id) }}"
                                            class="btn btn-sm btn-primary" title="Download">
                                            <i class="fas fa-download"></i>
                                        </a>
                                        <a href="{{ route('laporan.tahunan.preview', $laporan->id) }}"
                                    ...
                                    @if(canManage())
                                        <button class="btn btn-sm btn-warning"
                                            onclick="editLaporan({{ $laporan->id }}, '{{ $laporan->judul }}', '{{ $laporan->tanggal_laporan->format('Y-m-d') }}', '{{ $laporan->tahun }}')">
    ...
    @if(canManage())
            ...
            <div class="modal-dialog">
                <div class="modal-content">
                    <div class="modal-header" style="background: var(--asabri-navy); color: white;">
                        <h5 class="modal-title">Upload Laporan Tahunan</h5>
                    ...
                    <form action="{{ route('laporan.tahunan.store') }}" method="POST" enctype="multipart/form-data">
                        ...
                        <div class="modal-body">
                            ...
                            <div class="mb-3">
                                ...
                                <input type="number" name="tahun" class="form-control" placeholder="Contoh: 2026">
                            </div>
        ...
        <script>
            function editLaporan(id, judul, tanggal, tahun) {
                document.getElementById('editForm').action = "{{ url('laporan-tahunan') }}/" + id;
