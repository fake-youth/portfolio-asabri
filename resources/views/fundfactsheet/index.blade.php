<x-app-layout>
    <x-slot name="pageTitle">Fund Fact Sheet</x-slot>

    <div class="table-wrapper">
        <div class="d-flex justify-content-between align-items-center mb-4">
            <h5 class="mb-0" style="color: var(--asabri-navy);">
                <i class="fas fa-file-alt"></i> Daftar Fund Fact Sheet
            </h5>

            <div class="d-flex gap-2">
                <form action="{{ route('fundfactsheet.index') }}" method="GET" class="d-flex gap-2">
                    ...
                    @if(request('keyword') || request('date'))
                        <a href="{{ route('fundfactsheet.index') }}" class="btn btn-sm btn-secondary" title="Reset">
                    ...
                    <tbody>
                        @foreach($laporans as $index => $laporan)
                            <tr>
                                ...
                                <td>
                                    @auth
                                        <a href="{{ route('fundfactsheet.download', $laporan->id) }}" class="btn btn-sm btn-primary"
                                            title="Download">
                                            <i class="fas fa-download"></i>
                                        </a>
                                        <a href="{{ route('fundfactsheet.preview', $laporan->id) }}"
                                    ...
                                    @if(canManage())
                                        <button class="btn btn-sm btn-warning"
                                            onclick="editLaporan({{ $laporan->id }}, '{{ $laporan->judul }}', '{{ $laporan->tanggal_laporan->format('Y-m-d') }}')">
                                            <i class="fas fa-edit"></i>
                                        </button>

                                        <form action="{{ route('fundfactsheet.destroy', $laporan->id) }}" method="POST"
        ...
        @else
            <p class="text-center text-muted py-5">Belum ada dokumen yang diupload.</p>
        @endif
    </div>
    ...
    @if(canManage())
            ...
            <div class="modal-dialog">
                <div class="modal-content">
                    <div class="modal-header" style="background: var(--asabri-navy); color: white;">
                        <h5 class="modal-title">Upload Fund Fact Sheet</h5>
                        <button type="button" class="btn-close btn-close-white" data-bs-dismiss="modal"></button>
                    </div>
                    <form action="{{ route('fundfactsheet.store') }}" method="POST" enctype="multipart/form-data">
                        @csrf
        ...
        <div class="modal fade" id="editModal" tabindex="-1">
            <div class="modal-dialog">
                <div class="modal-content">
                    <div class="modal-header" style="background: var(--asabri-navy); color: white;">
                        <h5 class="modal-title">Edit Fund Fact Sheet</h5>
                        <button type="button" class="btn-close btn-close-white" data-bs-dismiss="modal"></button>
                    </div>
        ...
        <script>
            function editLaporan(id, judul, tanggal) {
                document.getElementById('editForm').action = "{{ url('fund-fact-sheet') }}/" + id;
